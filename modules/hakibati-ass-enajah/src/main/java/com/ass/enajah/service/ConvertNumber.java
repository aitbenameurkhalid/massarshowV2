package com.ass.enajah.service; //	ÈÓã Çááå ÇáÑÍãä ÇáÑÍíã

/**
 * Title: Convert
 *
 * <p>Description: Convert Number in letter (Arab and French)
 *
 * <p>Copyright: Copyright (c) 2020
 */
import java.awt.*;
...
public abstract class ConvertNumber {
  ...
  private static String traiter(String nombre) {
    ...
    nombre = nombre.replace(" ", "");
    if (nombre.charAt(0) == '-') nombre = nombre.substring(1);
    virgule = nombre.indexOf('.');
    frac = "";
    if (virgule > -1) {
      frac =
          nombre.substring(
              virgule + 1, ((nombre.length() - virgule) < 3 ? nombre.length() : virgule + 3));
      nombre = nombre.substring(0, virgule);
    }
    while (frac.length() < 2) frac += "0";
    if (nombre.length() > 18) nombre = "#Trop grand";
    else
      try {
  ...
  public static String AR(String nombre, String devise, String sou) {
    ...
    if (nombre.equals("#Erreur")) result = "\u062e\u0637\u0623#";
    else if (nombre.equals("#Trop grand")) result = "\u0643\u0628\u064a\u0631 \u062c\u062F\u0627#";
    else {
      result = "";
      while (rang > 0) {
        if (result == "") {
          tranche = nombre.substring(0, ((nombre.length() + 2) % 3) + 1);
          while (tranche.length() < 3) tranche = "0" + tranche;
        } else
          tranche =
              nombre.substring(nombre.length() - (rang * 3), nombre.length() - ((rang - 1) * 3));
        dec = tranche;
        ...
        switch (Integer.parseInt(cent)) {
          ...
          case 1:
            ...
            break;
          case 2:
            result += " \u0648\u0645\u0627\u0626\u062a\u0627\u0646"; // æãÇÆÊÇä
            break;
          default:
            result +=
                " \u0648" + n1[Integer.parseInt(cent)] + " \u0645\u0627\u0626\u0629"; // æ× ãÇÆÉ
        }
        switch (Integer.parseInt(dec)) {
          case 0:
            if (!cent.equals("0")) result += " " + n1000[(rang - 1) * 3];
            break;
          case 1:
            if (rang == 1) result += " \u0648" + n1[Integer.parseInt(dec)];
            else result += " \u0648" + n1000[(rang - 1) * 3];
            break;
          case 2:
            if (rang == 1) result += " \u0648" + n1[Integer.parseInt(dec)];
            else result += " \u0648" + n1000[((rang - 1) * 3) + 1];
            break;
          case 3:
          case 4:
          case 5:
          ...
          case 12:
            result +=
                " \u0648" + n1[Integer.parseInt(dec)] + " " + n10[1] + " " + n1000[(rang - 1) * 3];
            break;
          case 18:
            result +=
                " \u0648"
          ...
          case 19:
            result +=
                " \u0648" + n1[Integer.parseInt(un)] + " " + n10[1] + " " + n1000[(rang - 1) * 3];
            break;
          default:
            result +=
                (un.equals("0")
                    ? " \u0648" + n10[Integer.parseInt(dix)] + " " + n1000[(rang - 1) * 3]
                    : " \u0648"
                        + n1[Integer.parseInt(un)]
                        + " \u0648"
                        + n10[Integer.parseInt(dix)]
                        + " "
                        + n1000[(rang - 1) * 3]);
            break;
        }
      ...
      if (result.equals("")) result = "  \u0635\u0641\u0631 " + devise;
      else result += " " + devise;
  ...
  public static String FR(String nombre, String devise, String sou) {
    ...
    nombre = traiter(nombre);
    if ((nombre.equals("#Erreur")) || (nombre.equals("#Trop grand"))) result = nombre;
    else {
      result = "";
      while (rang > 0) {
        if (result == "") {
        ...
        switch (Integer.parseInt(cent)) {
          ...
          case 1:
            ...
            break;
          default:
            result += n1[Integer.parseInt(cent)] + " Cent";
        }
        switch (Integer.parseInt(dec)) {
          case 0:
            if (!cent.equals("0")) result += n1000[rang - 1];
            break;
          case 1:
          case 2:
          case 3:
          ...
          default:
            ...
            else result += n10[Integer.parseInt(dix)] + n1[Integer.parseInt(un)] + n1000[rang - 1];
        }
        rang--;
      }
      if (result.equals("")) result = " Zero " + devise + " et";
      else result += " " + devise + " et";
      un = Character.toString(frac.charAt(1));
      dix = Character.toString(frac.charAt(0));
      switch (Integer.parseInt(frac)) {
        case 0:
        ...
        default:
          ...
          else result += n10[Integer.parseInt(dix)] + n1[Integer.parseInt(un)] + " " + sou;
      }
  ...
  public static String FR(String nombre) {
    return FR(nombre, "Dirhams", "Centimes");
  }
}
